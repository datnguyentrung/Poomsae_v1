...
import './StudentSelectionModal.scss'
...
import { AgeGroupMap, BeltGroupMap, getDisplayName, PoomsaeContentMap } from '@/constants/TournamentConstants'
...
export default function StudentSelectionModal({ isOpen, onClose, onAddStudents, selectedCombination, listPoomsaeLists }: Props) {
    const [searchTerm, setSearchTerm] = React.useState('')
    // list of selected students
    const [selectedStudents, setSelectedStudents] = React.useState<StudentType[]>([])
    // list of poomsae DTOs
    const [listPoomsaeDTO, setListPoomsaeDTO] = React.useState<CompetitorDTOType[]>([])
    ...
    const filteredStudents = listStudents
        ...
        .filter(student =>
            ...
            // Nếu có selectedCombination thì kiểm tra không nằm trong listPoomsaeLists
            (selectedCombination
                ? !listPoomsaeLists.find(dto =>
    ...
    const handleAddStudent = (student: StudentType) => {
        ...
        setListPoomsaeDTO(prev => [...prev, {
            personalAcademicInfo: student,
            competition: {
                idPoomsaeCombination: selectedCombination?.idPoomsaeCombination ?? '',
            }
        }])
    }
    ...
    const handleRemoveStudent = (studentId: string) => {
        ...
        setListPoomsaeDTO(prev => prev.filter(dto => dto.personalAcademicInfo.personalInfo.idAccount !== studentId))
    }

    const handleSubmit = () => {
        onAddStudents(listPoomsaeDTO)
        setSelectedStudents([])
        setListPoomsaeDTO([])
        setSearchTerm('')
        onClose()
    ...
    const renderedSelectedCombination = selectedCombination
        ? [
            getDisplayName(PoomsaeContentMap, selectedCombination.poomsaeContent?.contentName ?? ''),
            getDisplayName(AgeGroupMap, selectedCombination.ageGroup?.ageGroupName ?? ''),
            getDisplayName(BeltGroupMap, selectedCombination.beltGroup?.beltGroupName ?? '')
        ]
            .filter(Boolean) // loại bỏ giá trị rỗng
            .join(' - ')     // nối thành chuỗi, ví dụ: "Form 1 - U12 - C9"
        : '';
